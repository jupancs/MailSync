...
public class FetchCommand extends SelectedStateCommand implements UidEnabledCommand {
  ...
  protected void doProcess(ImapRequestLineReader request,
                           ...
                           ImapSession session)
      throws ProtocolException, FolderException, MessagingException, IOException,
...
//                        ImapSession session,
//                        boolean useUids)
//      throws ProtocolException, FolderException, IOException, ClassNotFoundException,
//      MessagingException {
...
//    long[] uids = Arrays.copyOfRange(NDNMailSyncConsumerProducer.mailbox.messageUids, 0, end);
    ...
    System.out.println("Fetch Command - uids size: " + uids.length);
    ...
    for (long uid : uids) {
      ...
      if ((useUids && includes(idSet, uid)) ||
          (!useUids && includes(idSet, msn))) {
        String msgData = getMessageDataNdn(useUids, fetch, uid);
        System.out.println("msgDATA 1 =========== > " + msgData);
        response.fetchResponse(msn, msgData);
      }
    }
    ...
    // as per https://tools.ietf.org/html/rfc3501#section-6.4.8
    long lastMessageUid = uids.length > 0 ? uids[uids.length - 1] : -1L;
    if (NDNMailSyncConsumerProducer.mailbox.size > 0 && includes(idSet, Long.MAX_VALUE)
        && !includes(idSet, lastMessageUid)) {
      String msgData = getMessageDataNdn(useUids, fetch, lastMessageUid);
      System.out.println("msgDATA 2 =========== > " + msgData);
  ...
  private String getMessageDataNdn(boolean useUids, FetchRequest fetch, long uid)
      throws FolderException, ProtocolException, MessagingException {
    MimeMessage mimeMessage;
    if (NdnFolder.uidToMime.containsKey(uid)) {
      mimeMessage = NdnFolder.uidToMime.get(uid);
      mimeMessage = new MimeMessage(mimeMessage);
      System.out.println(uid + " MIMEMESSAGEEEEEEEEEEEEEE");
    } else {
      mimeMessage = null;
      System.out.println(uid + " NO MimeMessage-----------");
    }

    SimpleMessageAttributes attributes;
    if (mimeMessage != null) {
      attributes = NdnFolder.uidToAttr.get(uid);
      System.out.println(uid + " ATTRIBUTEEEEEEEEEEEEEEEE");
    } else {
      System.out.println(uid + " NO Attibute-----------");
      attributes = null;
    }
    return outputMessageNdn(fetch, mimeMessage, useUids, uid, attributes);
  }

  private String outputMessageNdn(FetchRequest fetch, MimeMessage message, boolean useUids,
                                  long uid, SimpleMessageAttributes attributes)
      throws FolderException, ProtocolException, MessagingException {
    ...
    if (fetch.isSetSeen() && !NdnFolder.uidToMime.get(uid).isSet(Flags.Flag.SEEN)) {
      //folder.setFlags(FLAGS_SEEN, true, uid, folder, useUids);
      NdnFolder.uidToMime.get(uid).setFlags(FLAGS_SEEN, true);
    ...
    if (message != null) {
      ...
      if (fetch.flags || ensureFlagsResponse) {
        ...
        response.append(MessageFlags.format(NdnFolder.uidToMime.get(uid).getFlags()));
      }
    } else {
      if (fetch.flags || ensureFlagsResponse) {
        response.append(" FLAGS ");
        response.append(MessageFlags.format(NDNMailSyncConsumerProducer.mailbox.flagMap.get(uid)));
      }
    }
    ...
    if (fetch.internalDate) {
      ...
      response.append(INTERNALDATE.format(NdnFolder.uidToMime.get(uid).getReceivedDate()));
      System.out.println("Date: " + INTERNALDATE.format(NdnFolder.uidToMime.get(uid).getReceivedDate()));
      response.append('\"');
    }
...
//    if (message != null) {
      ...
      response.append(NdnFolder.uidToMime.get(uid).getSize());
      System.out.println("Size: " + NdnFolder.uidToMime.get(uid).getSize());
    }
...
//    if (attributes != null) {
      ...
      System.out.println("Envelope: " + attributes.getEnvelope());
    }
...
//    if (attributes != null) {
      ...
      System.out.println("Body: " + attributes.getBodyStructure(false));
    }
...
//    if (attributes != null) {
      ...
      System.out.println("Body Structure: " + attributes.getBodyStructure(true));
    }
    ...
    for (BodyFetchElement fetchElement : elements) {
      ...
      MimeMessage mimeMessage = NdnFolder.uidToMime.get(uid);
      try {
